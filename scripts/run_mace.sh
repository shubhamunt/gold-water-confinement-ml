#!/bin/bash
#SBATCH --job-name=mace_baseline
#SBATCH --partition=gpu-preempt
#SBATCH --gpus=1
#SBATCH --constraint=a16
#SBATCH --mem=24G
#SBATCH --time=48:00:00
#SBATCH --output=mace_output_%j.txt
...
if [[ ! -f "${PROJECT_DIR}/src/inference_baseline.py" ]]; then
	...
	echo "[$(date)] Checked: ${SLURM_SUBMIT_DIR:-<unset>}/src and ${PROJECT_DIR}"
	exit 1
fi

cd "${PROJECT_DIR}"

echo "[$(date)] Starting MACE Job on Unity..."
...
echo "[$(date)] Running src/inference_baseline.py..."
python -u "${PROJECT_DIR}/src/inference_baseline.py"

LATEST_RUN_FILE="${PROJECT_DIR}/data/results/latest_run.txt"
SLURM_STDOUT_FILE="${PROJECT_DIR}/scripts/mace_output_${SLURM_JOB_ID:-}.txt"
